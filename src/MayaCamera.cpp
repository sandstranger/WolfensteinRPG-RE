...
void MayaCamera::Update(int i, int i2) {
    ...
    if (this->inheritX && mayaCameraKeys[game->OFS_MAYAKEY_X + n3 + 1] != -2) {
        this->x = (this->aggComponents[0] << 17) + n5 * (mayaCameraKeys[game->OFS_MAYAKEY_X + n3 + 1] - this->aggComponents[0] << 1) + 32768 >> 16;
    }
    if (this->inheritY && mayaCameraKeys[game->OFS_MAYAKEY_Y + n3 + 1] != -2) {
        this->y = (this->aggComponents[1] << 17) + n5 * (mayaCameraKeys[game->OFS_MAYAKEY_Y + n3 + 1] - this->aggComponents[1] << 1) + 32768 >> 16;
    }
    if (this->inheritZ && mayaCameraKeys[game->OFS_MAYAKEY_Z + n3 + 1] != -2) {
        this->z = (this->aggComponents[2] << 17) + n5 * (mayaCameraKeys[game->OFS_MAYAKEY_Z + n3 + 1] - this->aggComponents[2] << 1) + 32768 >> 16;
    }
...
void MayaCamera::Interpolate(int16_t* array, int i) {
    ...
    this->x = (this->aggComponents[0] << 17) + i * (array[0] << 1) + 32768 >> 16;
    this->y = (this->aggComponents[1] << 17) + i * (array[1] << 1) + 32768 >> 16;
    this->z = (this->aggComponents[2] << 17) + i * (array[2] << 1) + 32768 >> 16;
}
...
void MayaCamera::Render() {
    ...
    if (app->canvas->state == Canvas::ST_DIALOG) {
        app->render->render(this->x, this->y, this->z, this->yaw, this->pitch, this->roll, 281);
    }
    else {
        app->render->render(this->x, this->y, this->z, this->yaw, this->pitch, this->roll, 306);
    }
...
void MayaCamera::Snap(int i) {
    ...
    if (n2 + 1 < this->keyOffset + this->numKeys) {
        int n3 = n2 + 1;
        this->x = mayaCameraKeys[game->OFS_MAYAKEY_X + n3] << 1;
        this->y = mayaCameraKeys[game->OFS_MAYAKEY_Y + n3] << 1;
        this->z = mayaCameraKeys[game->OFS_MAYAKEY_Z + n3] << 1;
        this->pitch = mayaCameraKeys[game->OFS_MAYAKEY_PITCH + n3];
        this->yaw = mayaCameraKeys[game->OFS_MAYAKEY_YAW + n3];
        this->roll = mayaCameraKeys[game->OFS_MAYAKEY_ROLL + n3];
        if (this->inheritX && mayaCameraKeys[game->OFS_MAYAKEY_X + n3] == -2) {
            this->x = game->camPlayerX << 1;
        }
        if (this->inheritY && mayaCameraKeys[game->OFS_MAYAKEY_Y + n3] == -2) {
            this->y = game->camPlayerY << 1;
        }
        if (this->inheritZ && mayaCameraKeys[game->OFS_MAYAKEY_Z + n3] == -2) {
            this->z = game->camPlayerZ << 1;
        }
    ...
    if (canvas->state != Canvas::ST_CAMERA && canvas->state != Canvas::ST_DRIVING) {
        return;
    }
