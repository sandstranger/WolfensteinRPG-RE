...
#define FOG_SCALE(x)			(float)(1.f / (float)(x)) // (1.f / 16384) -> 0.000061035f

gles::gles() {
	memset(this, 0, sizeof(gles));
}
...
void gles::GLInit(Render* render) {
	...
	this->fogScale = FOG_SCALE(16384);
	memset(this->chains, 0, sizeof(this->chains));
...
void gles::BeginFrame(int x, int y, int w, int h, int* mtxView, int* mtxProjection) {
	...
	float fogRange = (((float)this->tinyGL->fogRange) / this->fogColor[3]);

	if (CAppContainer::getInstance()->app->game->isUnderWater() && fogRange < 3000.0) {
		fogRange = 3000.0;
	}

	this->fogStart = ((float)this->tinyGL->fogMin) * this->fogScale;
	this->fogEnd = (fogRange + (float)this->tinyGL->fogMin) * this->fogScale;
...
void gles::CreateFadeTexture(int mediaID) {
	...
	uint8_t* pixels = (uint8_t*)malloc((FADE_WIDTH * FADE_HEIGHT) * sizeof(int));
	memset(pixels, 0x00, (FADE_WIDTH * FADE_HEIGHT) * sizeof(int));
	int alpha = 255;
...
void gles::CreateTextureForMediaID(int n, int mediaID, bool b) {
	...
	else
	{
		...
		if (mediaID == render->mediaMappings[Enums::TILENUM_BLOOD_DECAL] && n == Enums::TILENUM_BLOOD_DECAL) { // [GEC]
			v9[0] = 0xF81F;
		}
	}
	v13 = 0;
	...
	do
	{
		...
		if ((v55[0] == 0) && (v55[1] >= 250) && (v55[2] >= 250)) {
			v55[0] = 0;
			v55[1] = 0;
			v55[2] = 0;
			v55[3] = 0;
