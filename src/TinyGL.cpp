	...
	while (i < 5) {
		...
		for (int n5 = 0; n5 < n; ++n5) {
			...
			if (this->cv[n5].clipDist < 0 != this->cv[n6].clipDist < 0) {
				...
				if (n7 > 0) {
					...
					memcpy(&this->cv[n5 + 2], &this->cv[n5 + 1], n7 * sizeof(TGLVert));
					memcpy(&this->cv[n5 + 1], tglVert, sizeof(TGLVert));
					++n6;
				}
		...
		for (int n9 = 0; n9 < n; ++n9) {
			...
			if (tglVert5->clipDist < 0) {
				memcpy(&this->cv[n9 + 0], &this->cv[n9 + 1], (n - 1 - n9) * sizeof(TGLVert));
				--n9;
				--n;
				memcpy(&this->cv[n], tglVert5, sizeof(TGLVert));
			}
		}
		if (n < 3) {
			return;
	...
	if (!app->render->_gles->RasterizeConvexPolygon(n, this->cv)) {
		for (int i = 0; i < n; ++i) {
			...
			if (app->render->isSkyMap) {
				...
				tglVert->s = app->render->skyMapX + (276 << 3) + tglVert->x << (this->sShift - 7U & 0xff);
				tglVert->t = app->render->skyMapY + (15  << 2) + tglVert->y << (this->tShift + 3U & 0xff);
				tglVert->z = 4096;
			}
			else {
				tglVert->z = 0x7FFFFF / tglVert->w;
...
bool TinyGL::clippedLineVisCheck(TGLVert* tglVert, TGLVert* tglVert2, bool b) {
	...
	// [GEC]
	// Cambio (tglVert->x + 7) a (tglVert->x + 3)
	// Cambio (tglVert2->x + 7) a (tglVert2->x + 3)
	// Reduce la posibilidad de renderizar partes fuera del nodo
	//
	// Change (tglVert->x + 7) to (tglVert->x + 3)
	// Change (tglVert2->x + 7) to (tglVert2->x + 3)
	// Reduces the possibility of rendering parts outside the node

	int begX = tglVert->x + 3 >> 3;
	int endX = tglVert2->x + 3 >> 3;
	if (begX < 0) {
		begX = 0;
	}
...
void TinyGL::applyClearColorBuffer() {
	...
	for (int j = 1; j < this->viewportHeight; ++j) {
		n3 += this->screenWidth;
		memcpy(&pixels[n3], &pixels[n2], this->viewportWidth * sizeof(uint16_t));
	}
}
