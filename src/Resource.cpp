...
constexpr char* Resources::RES_NEWTEXEL_FILE_ARRAY[];
...
Resource::Resource() {
    ...
    memset(this->ioBuffer, 0, Resource::IO_SIZE);
}
...
int* Resource::loadFileIndex(char* fileName) {
    ...
    /*for (int j = 0; j < count; j++) {
        printf("array[(%d * 3) + 0] %d\n", j, array[((j) * 3) + 0]);
        printf("array[(%d * 3) + 1] %d\n", j, array[((j) * 3) + 1]);
        printf("array[(%d * 3) + 2] %d\n", j, array[((j) * 3) + 2]);
    }*/
...
void Resource::beginTableLoading() {
    ...
    this->bufSkip(&this->prevIS, 128, false);
    this->prevOffset = 0;
}
...
void Resource::loadShortTable(short* array, int index) {
    this->seekTable(index);
    this->read(&this->prevIS, sizeof(int));
