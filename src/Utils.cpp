...
void fixImage(Image* img) {
    ...
    if (checkFileMD5Hash(piDIB->pBmp, size, 0x40CCEBD45EFC0C67, 0xE9D6E9571FB7B0A9)) { // "blockGameColors.bmp"
        ...
        uint8_t* data = (uint8_t*)malloc(newW * h);
        memset(data, 0, newW * h);

        for (int i = 0; i < h; i++) { // Copy all image data
            memcpy(data + (i * newW), piDIB->pBmp + (i * w), w);
        }
        ...
        for (int i = 0; i < fixH; i++) { // Copy the corrected snippet of the image.
            memcpy((data + (i * newW)) + fixW, fixData + (i * fixW), fixW);
        }
...
void enlargeButtonImage(Image* img) {
    ...
    for (int i = 0; i < h; i++) {
        memcpy((data + (i * newW) + 198), piDIB->pBmp + (i * w) + 140, 142);
    }
...
void enlargeVideoBOXImage(Image* img) {
    ...
    int newW = 342;
    int newH = 220;
    uint8_t* data = (uint8_t*)malloc(newW * newH);
    memset(data, 0x00, newW * newH);
    ...
    for (int i = 0; i < h; i++) {
        memcpy((data + (i * newW) + 120), piDIB->pBmp + (i * w) + 62, 222);
    }

    for (int i = 0; i < 40; i++) {
        memcpy(data + ((218 - i) * newW), data + ((181 - i) * newW), 342);
    }
...
void setPlaqueImage(Image* img) {
    ...
    for (int i = 0; i < newH; i++) { // Copy all image data
        memcpy(data + (i * newW), piDIB->pBmp + ((i + 0) * w), w);
    }
...
void setEmptyPlaqueImage(Image* img) {
    ...
    for (int i = 0; i < newH; i++) { // Copy all image data
        memcpy(data + (i * newW), piDIB->pBmp + ((i + 36) * w), w);
    }
...
void setEnlargeEmptyPlaqueImage(Image* img) {
    ...
    for (int i = 0; i < newH; i++) { // Copy all image data
        //memcpy(data + (i * newW), piDIB->pBmp + ((i + 36) * w), w);
        memcpy(data + (i * newW), piDIB->pBmp + ((i + 36) * w), 230);
        memcpy(data + (i * newW) + 228, piDIB->pBmp + ((i + 36) * w) + 170, 114);
    }
...
void setSliderBarImage(Image* img) {
    ...
    for (int i = 0; i < newH; i++) { // Copy all image data
        memcpy(data + (i * newW), piDIB->pBmp + ((i + 146) * w), w);
    }
